  ...
  box.innerHTML = entries.map((en, idx) => {
    ...
    return `<div class="selected-item-badge">
      <span class="item-label"><i class="ti ${iconFor(en.type)}"></i>${esc(en.type)}
        ${entries.length > 1 ? `<span class="text-muted small"> (w:${en.spawnWeight})</span>` : ''}${sub}</span>
  ...
  body.innerHTML = `
    <div class="row g-3 mb-3">
      <div class="col-6 col-md-3"><label class="form-label small text-muted">Spawn weight</label>
        <input type="number" class="form-control form-control-sm" id="cfg-weight" min="0" step="1" value="${entry.spawnWeight}"></div>
      ...
      <div class="col-6 col-md-3"><label class="form-label small text-muted">Health min (0–1)</label>
        <input type="number" class="form-control form-control-sm" id="cfg-hmin" min="0" max="1" step="0.05" value="${entry.healthMin}"></div>
      <div class="col-6 col-md-3"><label class="form-label small text-muted">Health max (0–1)</label>
        <input type="number" class="form-control form-control-sm" id="cfg-hmax" min="0" max="1" step="0.05" value="${entry.healthMax}"></div>
      <div class="col-6 col-md-3"><label class="form-label small text-muted">Quantity min</label>
        <input type="number" class="form-control form-control-sm" id="cfg-qmin" min="-1" max="1" step="0.05" value="${entry.quantityMin}"></div>
      <div class="col-6 col-md-3"><label class="form-label small text-muted">Quantity max</label>
        <input type="number" class="form-control form-control-sm" id="cfg-qmax" min="-1" max="1" step="0.05" value="${entry.quantityMax}"></div>
...
function entryFromSet(set) {
  const en = defaultEntry(set.itemType || set.type || 'Unknown');
  en.spawnWeight = set.spawnWeight != null ? set.spawnWeight : 1;
  en.quickBarSlot = set.quickBarSlot != null ? set.quickBarSlot : -1;
  const a = set.attributes || {};
  if (a.healthMin != null) en.healthMin = a.healthMin;
  if (a.healthMax != null) en.healthMax = a.healthMax;
  if (a.quantityMin != null) en.quantityMin = a.quantityMin;
  if (a.quantityMax != null) en.quantityMax = a.quantityMax;
  const it = ITEM_BY_NAME[en.type];
  const known = new Set(it && it.sa ? it.sa : []);
  (set.simpleChildrenTypes || []).forEach(t => (known.has(t) ? en.simple : en.extra).push(t));
  (set.complexChildrenSets || set.complexChildrenTypes || []).forEach(c => {
    if (typeof c === 'string') en.complex.push({ type: c, children: [] });
    else if (c && c.itemType) en.complex.push({ type: c.itemType, children: c.simpleChildrenTypes || [] });
  });
  return en;
}
...
function loadFromJson(data) {
  try {
    ...
    state.name = data.name || 'Imported Loadout';
    state.spawnWeight = data.spawnWeight != null ? data.spawnWeight : 1;
    state.characterTypes = Array.isArray(data.characterTypes) ? data.characterTypes.slice() : [];
    SLOTS.forEach(s => { if (s.key !== 'Cargo') state.slots[s.key] = []; });
    state.cargo = [];
    (data.attachmentSlotItemSets || []).forEach(slotSet => {
      let key = slotSet.slotName;
      ...
      if (!state.slots[key]) {
        ...
        (slotSet.discreteItemSets || []).forEach(set => state.cargo.push(entryFromSet(set)));
        return;
      }
      (slotSet.discreteItemSets || []).forEach(set => state.slots[key].push(entryFromSet(set)));
    });
    (data.discreteUnsortedItemSets || []).forEach(us => {
      (us.simpleChildrenTypes || []).forEach(t => state.cargo.push(defaultEntry(t)));
      (us.complexChildrenSets || us.complexChildrenTypes || []).forEach(c => {
        if (typeof c === 'string') state.cargo.push(defaultEntry(c));
        else if (c && c.itemType) state.cargo.push(entryFromSet(c));
      });
    });
